    ...
    const fetchImageInfo = async () => {
      try {
        ...
        if (response.data.status === "success") {
          ...
          if (image.unique) {
            const imageFileResponse = await httpClient.post(
              "images/get-file",
              {
  ...
  return (
    <div className="flex flex-row flex-wrap w-full h-full p-4 ">
      ...
      <div className="basis-1/3 flex-grow p-2 flex flex-col mt-8">
        ...
        {imgFile && (
          <div className="result-item w-full h-96  sm:md:h-80 lg:h-96 xl:h-96  border-b-[1px] border-gray-400 pb-8 mt-4">
            <p>
              {imageInfo
                ? `The image is classified as: ${imageInfo.classified_as}`
                : "Uploading..."}
            </p>
